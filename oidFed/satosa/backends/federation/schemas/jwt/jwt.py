...
class JWTConfig(BaseModel):
    ...
    access_token_exp: Optional[int] = Field(
        None, gt=0
    )  # access_token_exp must be greater than 0
    refresh_token_exp: Optional[int] = Field(
        None, gt=0
    )  # refresh_token_exp must be greater than 0
